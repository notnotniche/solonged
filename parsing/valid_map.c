...
/*   Updated: 2023/05/12 18:58:47 by nklingsh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_size_map_word(int len, char *map_str, char *valid_word)
{
	...
	{
		if (size_str(str) != len)
			return (close(fd), free(str), printf("Error : taille de map"), (0));
		if (check_word_in_str(str, valid_word) == 0)
			return (close(fd), free(str), printf("Error: lettres"), 0);
		free(str);
		str = get_next_line(fd);
	}
...
int	valid_map(char *valid_word, char *map_str, char *line)
{
	...
	if (len == 0)
		return (printf("Error : map cant have 0 line"), 0);
	...
	if (check_world_of_wall(map_str, len) == 0)
		return (printf("Error : world of wall "), 0);
